...
class IsDefined(Filter):
    ...
    def __call__(self, data):
        ...
        # Vectorised fallback: avoid per-instance Python loop
        try:
            X = np.asarray(data.X, dtype=float)
            r = ~np.any(np.isnan(X), axis=1)
        except Exception:
...
class HasClass(Filter):
    ...
    def __call__(self, data):
        ...
        try:
            ...
            r = np.fromiter((not bn.anynan(inst._y) for inst in data),
                            bool, len(data))
        if self.negate:
            r = np.logical_not(r)
        return data[r]
...
class SameValue(Filter):
    ...
    def __call__(self, data):
        ...
        # Vectorised fallback: use numpy column slicing instead of per-row loop
        try:
            if column >= 0:
                col_vals = data.X[:, column] if column < data.X.shape[1] \
                    else data.Y if data.Y.ndim == 1 \
                    else data.Y[:, column - data.X.shape[1]]
            else:
                meta_col = -1 - column
                col_vals = data.metas[:, meta_col]
            retain = (col_vals != value) if self.negate else (col_vals == value)
        except Exception:
            if column >= 0:
                if self.negate:
                    retain = np.fromiter(
