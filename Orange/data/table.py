...
class Table(Sequence, Storage):
    ...
    def transform(self, domain):
        ...
        Returns:
            A new table
        """
        # Fast path: if the domain IS the same object, return self immediately.
        # For equal-but-distinct domain objects, still use from_table so the
        # returned table gets the new domain object (test_transform_same_domain).
    ...
    def transpose(cls, table, feature_names_column="",
        ...
        with self.unlocked_reference():
            ...
            if feature_names_column:
                # Fast path: vectorised column extraction instead of
                # per-row Python iteration (was O(n) Orange row objects).
                # Use str_val per element to match original str(row[var]) behaviour
                # (e.g. ContinuousVariable.str_val formats 1.0 → "1", not "1.0").
                _fcvar = table.domain[feature_names_column]
                _fcol = table.get_column(feature_names_column)
                names = [_fcvar.str_val(v) for v in _fcol]
            ...
            def set_attributes_of_attributes(_vars, _table):
                # Vectorised: extract each variable column as numpy array,
                # then assign to attribute dicts in one Python pass.
                # Avoids O(rows × vars) Orange row-object overhead.
                # Use str_val — matches original str(row[var]) behaviour.
                # (str_val, NOT repr_val: strings stored without quotes.)
                _arr = np.asarray(_table)
                for i, variable in enumerate(_vars):
                    if variable.name == feature_names_column:
                        continue
                    col = _arr[:, i] if _arr.ndim > 1 else _arr
                    for j, val in enumerate(col):
                        value = variable.str_val(val)
    ...
    def groupby(self, columns: List[Variable]) -> "OrangeTableGroupBy":
        ...
        Uses the Polars-accelerated backend by default for significantly
        faster aggregation on large tables.
        ...
        groups.
        """
        # Normalize single Variable to a list so both backends can iterate
        if isinstance(columns, Variable):
            columns = [columns]
        try:
            from Orange.data.aggregate import OrangeTableGroupByPolars
            return OrangeTableGroupByPolars(self, columns)
        except Exception:
            pass
    ...
    def to_arrow(self, include_metas: bool = True):
        """
        Convert this table to a PyArrow Table (columnar, zero-copy where
        possible).
        ...
        pyarrow.Table
        """
        from Orange.data.arrow_backend import table_to_arrow
        return table_to_arrow(self, include_metas=include_metas)

    @classmethod
    def from_arrow(cls, arrow_table, domain=None):
        """
        Construct an Orange Table from a PyArrow Table.

        Zero-copy numpy views are used wherever the Arrow buffers allow it.
        ...
        domain : Domain, optional
            Target domain.  Inferred from Arrow schema when omitted.

        Returns
        -------
        Table
        """
        from Orange.data.arrow_backend import table_from_arrow
        tbl = table_from_arrow(arrow_table, domain=domain)
        if cls is not Table:
            # Preserve subclass by re-constructing through from_numpy
            try:
                tbl = cls.from_numpy(tbl.domain, tbl.X, tbl.Y, tbl.metas, tbl.W if tbl.has_weights() else None)
            except Exception:
                pass
        return tbl

    def to_polars(self, include_metas: bool = True):
        """
        ...
        return table_to_polars(self, include_metas=include_metas)

    @classmethod
    def from_polars(cls, df, domain=None):
        """
        Construct an Orange Table from a Polars DataFrame.

        Parameters
        ----------
        df : polars.DataFrame
        domain : Domain, optional
            If provided, variables in the domain override type inference.

        Returns
        -------
        Table
        """
        from Orange.data.polars_compat import table_from_polars
        tbl = table_from_polars(df)
        if domain is not None:
            tbl = cls.from_table(domain, tbl)
        elif cls is not Table:
            try:
                tbl = cls.from_numpy(tbl.domain, tbl.X, tbl.Y, tbl.metas,
                                     tbl.W if tbl.has_weights() else None)
            except Exception:
                pass
        return tbl

    def filter_duckdb(self, sql_where: str):
        """
        Filter table rows using a DuckDB SQL WHERE clause for maximum
        performance on large datasets.

        This is the fastest filter path for large tables (>100 K rows) when
        the filter can be expressed as SQL.

        Parameters
        ----------
        sql_where : str
            A valid DuckDB SQL WHERE expression applied to columns named by
            their Orange variable names, e.g.::
        ...
        -------
        Table
            A new table of the same type containing only the matching rows.

        Raises
        ------
        RuntimeError
            If ``pyarrow`` or ``duckdb`` are not installed.
        """
        from Orange.data.arrow_backend import duckdb_filter_table
        result = duckdb_filter_table(self, sql_where)
        if type(self) is not Table:
            try:
                result = type(self).from_numpy(
                    result.domain, result.X, result.Y, result.metas,
                    result.W if result.has_weights() else None,
                )
            except Exception:
                pass
        return result
    ...
    def to_parquet(self, path: str, compression: str = "snappy"):
        """
        Write this table to a Parquet file via Arrow (efficient columnar storage).
        ...
        ----------
        path : str
            Destination file path (e.g. ``'/data/iris.parquet'``)
        compression : str
            Parquet compression codec – one of ``'snappy'``, ``'gzip'``,
            ``'brotli'``, ``'zstd'``, or ``'none'``.
        """
        import pyarrow.parquet as pq
        arrow_tbl = self.to_arrow(include_metas=True)
        pq.write_table(arrow_tbl, path, compression=compression)

    @classmethod
    def from_parquet(cls, path: str, domain=None):
        """
        Load a Parquet file into an Orange Table via Arrow (fast columnar read).
        ...
        path : str
            Source file path
        domain : Domain, optional
            Target domain.  Inferred from file schema when omitted.
        ...
        return cls.from_arrow(arrow_tbl, domain=domain)
