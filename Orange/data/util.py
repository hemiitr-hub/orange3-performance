...
def get_unique_names_duplicates(proposed: list, return_duplicated=False) -> list:
    ...
    Optimised for large lists: O(n) single pass instead of the original
    O(n × d) where d = number of distinct duplicate names.  The original
    called get_indices() — a full regex scan of the entire list — once per
    duplicate name, making it quadratic on large inputs (e.g. 4.5 M rows).
    The new approach scans once to find the maximum existing suffix index for
    each base name, then assigns new suffixes in a second O(n) pass.
    """
    # Pass 1: find max existing parenthesised index for every base name.
    # e.g. "x (2)" → base "x", max_idx["x"] = 2.
    # Plain occurrences of "x" don't affect max_idx (they are index 0 implicitly).
    max_idx: dict = {}   # base_name → max seen suffix index (int)
    raw_count: dict = Counter(proposed)
    ...
    for name in proposed:
        ...
        if m:
            ...
            max_idx[base] = max(max_idx.get(base, 0), idx)

    # Start new indices right after the highest existing suffix index.
    # For a plain duplicate like ["a","a","a"] with no existing suffixes,
    # max_idx.get("a", 0) == 0, so counter starts at 1 → (1), (2), (3).
    indices = {name: count(max_idx.get(name, 0) + 1)
               for name, cnt in raw_count.items()
               if name and cnt > 1}
