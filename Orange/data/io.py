...
class CSVReader(FileFormat, DataTableMixin):
    ...
    # File extensions that support the Polars fast-path (non-annotated files)
    _POLARS_EXTENSIONS = ('.csv', '.tsv')

    def read(self):
        # FAST PATH: Attempt to load with Polars if available for CSV/TSV files
        # that don't have Orange-specific type annotation header rows.
        _ext = path.splitext(self.filename)[-1].lower()
        if pl is not None and _ext in self._POLARS_EXTENSIONS:
            # Check for Orange-specific type annotation lines (rows 2 and 3)
            has_annotations = False
            delimiter = self.DELIMITERS[0] if self.DELIMITERS else ','
            try:
                with self.open(self.filename, mode='rt', encoding='utf-8') as file:
                    first_lines = [file.readline() for _ in range(3)]
                    if len(first_lines) >= 3:
                        _annotation_tokens = frozenset({
                            'd', 'c', 't', 'string', 'discrete', 'continuous',
                            'time', 'class', 'meta', 'ignore', 'weight',
                        })
                        for line in first_lines[1:3]:
                            cells = line.strip().split(delimiter)
                            if any(c.strip().lower() in _annotation_tokens
                                   for c in cells):
            ...
            if not has_annotations:
                try:
                    ...
                    # Fallback to python standard parser on any error
                    pass
...
class TabReader(CSVReader):
    ...
    DELIMITERS = '\t'
    PRIORITY = 10
    # .tsv files without annotation rows can use the Polars fast-path
    _POLARS_EXTENSIONS = ('.tsv',)
