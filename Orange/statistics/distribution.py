...
class Discrete(Distribution):
    ...
    def modus(self):
        if self.shape[0] == 0:
            # Empty distribution: variable has no defined values or all values are
            # missing. Return NaN so callers can detect and skip imputation.
            return data.Value(self.variable, float('nan')) \
                if self.variable is not None else float('nan')
        val = np.argmax(self)
