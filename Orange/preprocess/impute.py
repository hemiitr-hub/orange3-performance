    ...
    def __call__(self, data, variable, value=None):
        ...
        if value is None:
            if variable.is_continuous:
                ...
                value = dist.modus()
                # If the distribution is empty (all-NaN column or variable with
                # no defined values), modus() returns NaN. Fall back to the
                # first category value so ReplaceUnknowns gets a valid sentinel.
                import math
                try:
                    _v = float(value)
                    ...
                    _v = float('nan')
                if math.isnan(_v):
                    value = variable.values[0] if variable.values else float('nan')
            else:
