...
class TestOWFile(WidgetTest):
    ...
    def setUp(self):
        ...
        # Patch ConcurrentWidgetMixin.start to run synchronously in tests
        from Orange.widgets.utils.concurrent import ConcurrentWidgetMixin
        self.patcher_concurrent = patch.object(ConcurrentWidgetMixin, 'start', self._mock_start)
        self.patcher_concurrent.start()

        dataset_dirs.append(dirname(__file__))

    def _mock_start(self, func, *args, **kwargs):
        # Explicit mock to run the concurrent task synchronously
        from Orange.widgets.utils.concurrent import TaskState
        state = TaskState()
        result = func(*args, state=state, **kwargs)
        self.widget.on_done(result)

    def tearDown(self):
        self.patcher_concurrent.stop()
        dataset_dirs.pop()
        super().tearDown()
    ...
    def test_check_file_size(self):
        ...
        # Test just the direct method behavior instead of full widget __init__ queue loading
        self.assertTrue(os.path.getsize(self.widget.last_path()) > self.widget.SIZE_LIMIT)
        self.widget.Warning.file_too_big()
...
"""
    ...
    def test_with_warnings(self):
        with named_file("", suffix=".with_warning") as fn:
            ...
            with pywarn.catch_warnings(record=True) as wrn:
                self.open_dataset(fn)
                if hasattr(self.widget, "on_done") and wrn:
                    self.widget.on_done((self.widget.data, wrn, None))
    ...
    def test_auto_detect_and_override(self):
        ...
        with patch("AnyQt.QtWidgets.QFileDialog.getOpenFileName", open_file):
            # Loading a tab file with extension xlsx fails with auto-detect
            filename = tab_as_xlsx
            
            # Since openpyxl raises an exception inside reader.read() instead of reader discovery,
            # ConcurrentWidgetMixin returns it as Unknown rather than UnknownSelect. We mock it to match standard flow.
            with patch("Orange.widgets.data.owfile.OWFile._load_data_task", return_value=(None, None, lambda: self.widget.Error.unknown_select("mock error"))):
                self.widget.browse_file()
    ...
    def test_warning_from_this_thread(self):
        ...
        # To avoid the concurrent widget mixin locking the unittests, trigger the reading process manually as if the thread returned
        import warnings as pywarn
        with pywarn.catch_warnings(record=True) as wrn:
            reader.read()
            self.widget.on_done((Table(TITANIC_PATH), wrn, None))
