...
class _TableModel(RichTableModel):
    ...
    def setSubsetRowIds(self, subsetids: Container[int]):
        ...
        # Clear cache as subset might change mapping?
        # Actually _TableModel doesn't seem to have sorting/filtering proxy in its own mapToSourceRows
    ...
    def _get_source_item(self, row, coldesc):
        if not isinstance(self.source, SqlTable):
            return super()._get_source_item(row, coldesc)

        # Determine the chunk that contains this row
        chunk_size = 500
        chunk_start = (row // chunk_size) * chunk_size

        if chunk_start != self._last_chunk_start:
            # Create a shallow copy
            chunk_table = self.source.copy()
            # Download only this chunk using the new offset parameter
            chunk_table.download_data(limit=chunk_size, partial=True, offset=chunk_start)
            # IMPORTANT: Convert to memory table so that index access 
            # (e.g. [relative_row]) uses the downloaded buffer correctly.
            from Orange.data import Table
            self._current_chunk = Table.from_table(self.source.domain, chunk_table)
