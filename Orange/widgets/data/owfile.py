...
class OWFile(OWUrlDropBase, RecentPathsWComboMixin, ConcurrentWidgetMixin):
    name = "File"
    ...
    def _try_load(self):
        ...
        self.start(self._load_data_task, self.reader)
        return None

    @staticmethod
    def _load_data_task(reader: FileFormat, state: TaskState):
        with log_warnings() as warnings:
            try:
                data = reader.read()
            except Exception as ex:
                log.exception(ex)
                return None, warnings, lambda self, x=ex: self.Error.unknown(str(x))
            return data, warnings, None

    def on_done(self, result):
        if result is None:
             return
        data, warnings, error = result
        if error:
            error(self)
            self.data = None
            self.sheet_box.hide()
    ...
    def on_exception(self, ext):
        self.setBlocking(False)
        self.Error.unknown(str(ext))
        self.data = None
        self.sheet_box.hide()
        self.Outputs.data.send(None)
        self.infolabel.setText("No data.")
