...
from AnyQt.QtCore import Qt, QSize, QRectF, QModelIndex, pyqtSlot, \
    QItemSelection, QItemSelectionRange, QItemSelectionModel, QTimer
...
class FeatureStatisticsTableModel(AbstractSortTableModel):
    ...
    def set_data(self, data):
        ...
        self.endResetModel()
        # Pre-warm histogram cache during idle time to avoid first-scroll stutter
        QTimer.singleShot(0, self._prewarm_distributions_cache)
    ...
    def set_target_var(self, variable):
        ...
        # Pre-warm histogram cache after target changes
        QTimer.singleShot(0, self._prewarm_distributions_cache)

    def _prewarm_distributions_cache(self):
        """Pre-compute all histogram scenes during idle time to avoid first-scroll stutter."""
        if self.table is None:
            return
        table = self.table
        target_var = self.target_var
        for row, attribute in enumerate(self.variables):
