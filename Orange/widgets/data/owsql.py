...
class OWSql(OWBaseSql):
    ...
    def __backend_changed(self):
        ...
        is_duckdb = self.selected_backend == "DuckDB"
        
        if hasattr(self, 'browse_button'):
            self.browse_button.setVisible(is_duckdb)
            
        if is_duckdb:
            self.servertext.setVisible(False)
            self.usernametext.setVisible(False)
            self.passwordtext.setVisible(False)
            self.databasetext.setPlaceholderText("File Path or :memory:")
            self.databasetext.setToolTip("Path to Parquet/CSV file, or :memory:")
        else:
            self.servertext.setVisible(True)
    ...
    def get_table(self):
        ...
        try:
            if getattr(self.backend, 'display_name', '') == "DuckDB":
                from Orange.data.sql.duckdb_table import DuckDBTable
                table = DuckDBTable(what,
