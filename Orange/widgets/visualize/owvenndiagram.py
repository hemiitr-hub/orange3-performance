...
class OWVennDiagram(widget.OWWidget):
    ...
    @staticmethod
    def _hashes(table):
        # Vectorised: hash each row's concatenated bytes without creating
        # Python Instance objects.  XOR the row-hashes of X, Y, metas arrays.
        # Falls back to the old per-instance path only for edge cases (empty
        ...
        try:
            def _row_hashes(arr):
                ...
                # Hash each row's raw bytes
                return np.array([hash(row.tobytes()) for row in arr],
                                dtype=np.int64)
            hx = _row_hashes(table.X)
            hy = _row_hashes(table.Y if table.Y.ndim == 2 else
                             table.Y[:, np.newaxis] if table.Y.size else None)
            hm = _row_hashes(np.asarray(table.metas, dtype=object)
                             if table.metas.size else None)
            return list(map(int, hx ^ hy ^ hm))
        except Exception:
            # safe fallback
    ...
    def _itemsForInput(self, key):
        ...
        # Vectorised: use get_column instead of per-instance iteration
        col = table.get_column(attr)
        var = table.domain[attr]
        if var.is_primitive():
            # Continuous/discrete: NaN marks missing values
            mask = ~np.isnan(col.astype(float))
        else:
            # String/other: empty string or "?" marks missing
            mask = np.array([bool(v) and v != "?" for v in col], dtype=bool)
        return [str(v) for v in col[mask]]
