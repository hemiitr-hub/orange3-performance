...
class OWScatterPlotBase(gui.OWComponent, QObject):
    ...
    def __init__(self, scatter_widget, parent=None, view_box=ViewBox):
        ...
        self._adaptive_timer = QTimer(self, singleShot=True, interval=100)
        self._adaptive_timer.timeout.connect(self._on_adaptive_timeout)
    ...
    def _create_sample(self):
        ...
        if self.adaptive_sampling:
            ...
            if x is not None:
                ...
                # If we don't have a plot yet, we need a global sample to establish initial range
                if self.scatterplot_item is None:
                    limit = self.sample_size or 50000
                    self.n_shown = min(len(x), limit)
                    random = np.random.RandomState(seed=0)
                    self.sample_indices = random.choice(len(x), self.n_shown, replace=False)
                else:
                    # Viewport specific sample
                    mask = (x >= xmin) & (x <= xmax) & (y >= ymin) & (y <= ymax)
                    idx = np.where(mask)[0]
                    limit = self.sample_size or 50000
                    if len(idx) > limit:
                        random = np.random.RandomState(seed=0)
                        self.sample_indices = idx[random.choice(len(idx), limit, replace=False)]
                    else:
                        self.sample_indices = idx
                self.n_shown = len(self.sample_indices)
                return
    ...
    def update_coordinates(self):
        ...
        # In adaptive mode, it's normal to have 0 points in view.
        # We must still proceed to update (clear) the plot.

        if self.scatterplot_item is None:
            if len(x) == 0 and not self.adaptive_sampling:
                return
            
            # Reset view only the first time data is set
            self._reset_view(x, y)

            indices = self.sample_indices if self.sample_indices is not None else np.arange(self.n_valid)
            ...
            if len(self.scatterplot_item.data) == len(x) and len(x) > 0:
                self.scatterplot_item.setCoordinates(x, y)
                self.scatterplot_item_sel.setCoordinates(x, y)
            else:
    ...
    def update_sizes(self):
        ...
        if self.scatterplot_item:
            ...
            # Robust count check for adaptive mode
            current_count = len(self.scatterplot_item.data)
            if len(size_data) != current_count:
                # If counts don't match, it means update_coordinates 
                # hasn't caught up or we're in a race. Skip or force sync.
                if self.adaptive_sampling:
                    return # Will be caught by the next adaptive timer
                else:
            ...
            if len(size_data) != len(current_size_data):
                self.scatterplot_item.setSize(size_data)
                self.scatterplot_item_sel.setSize(size_data + SELECTION_WIDTH)
                return
    ...
    def update_colors(self):
        ...
        if self.scatterplot_item is not None:
            ...
            if len(pen_data) == len(self.scatterplot_item.data):
    ...
    def update_shapes(self):
        ...
        if self.scatterplot_item:
            shape_data = self.get_shapes()
            if len(shape_data) == len(self.scatterplot_item.data):
                self.scatterplot_item.setSymbol(shape_data)
        self.update_legends()
