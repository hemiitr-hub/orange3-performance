...
class OWScatterPlot(OWDataProjectionWidget, VizRankMixin(ScatterPlotVizRank)):
    ...
    def _add_controls_sampling(self):
        box = gui.vBox(self.controlArea, "Sampling")
        self.sampling = gui.auto_commit(
            box, self, "auto_sample", "Sample",
            callback=self.switch_sampling, commit=lambda: self.add_data(1))
        box.setVisible(False)
        self.sampling_box = box
        gui.checkBox(box, self, "graph.adaptive_sampling",
                     "Adaptive viewport sampling",
                     callback=self.setup_plot,
                     tooltip="Resample data on zoom/pan to maintain performance")
    ...
    def check_data(self):
        ...
        if isinstance(self.data, SqlTable):
            if self.data.approx_len() < 4000:
                ...
                if self.auto_sample:
                    self.__timer.start()
        elif self.data is not None and len(self.data) > 50000:
            self.sampling_box.setVisible(True)
