...
from AnyQt.QtCore import Qt, QSize, QLineF, QTimer, pyqtSignal as Signal
...
class LinePlotViewBox(ViewBox):
    ...
    def add_profiles(self, y):
        if sp.issparse(y):
            y = y.todense()
        y = np.asarray(y)          # m instances × n features
        m, n = y.shape
        if m > MAX_LINES:
            rng = np.random.default_rng(0)
            idx = np.sort(rng.choice(m, MAX_LINES, replace=False))
            y = y[idx]
            m = MAX_LINES
        # build (n_features, n_instances, 2) without a Python loop
        xs = np.arange(1, n + 1, dtype=float)  # (n,)
        xs_tiled = np.broadcast_to(xs[:, None], (n, m))   # (n, m)
        self._profile_items = np.stack([xs_tiled, y.T], axis=2)  # (n, m, 2)
    ...
    def wheelEvent(self, ev, axis=None):
        # Ctrl+wheel → zoom Y; plain wheel → pan Y (triggers lazy line reload)
        delta = ev.delta()
        if ev.modifiers() & Qt.ControlModifier:
            factor = 1.15 ** (-delta / 120)
            self.scaleBy((1, factor))
        else:
            y_min, y_max = self.viewRange()[1]
            shift = (y_max - y_min) * 0.1 * (1 if delta > 0 else -1)
            self.translateBy(y=shift)
        ev.accept()
...
class ProfileGroup:
    def __init__(self, data, indices, color, graph):
        ...
        self.graph_items = []
        # densify once; stats and viewport filtering both need random access
        self._dense_y = np.asarray(
            self.y_data.todense() if sp.issparse(self.y_data) else self.y_data
        )
        # stats always from full data
        self.__mean = nanmean(self._dense_y, axis=0)
        # initial display: full-range sample
        self._y_display = self._viewport_sample(None, None)
    ...
    def _get_profiles_curve(self):
        x, y, con = self.__get_disconnected_curve_data(self._y_display)
    ...
    def _get_missing_profiles_curve(self):
        x, y, con = self.__get_disconnected_curve_missing_data(self._y_display)
        pen = self.make_pen(self.color)
        curve = pg.PlotCurveItem(x=x, y=y, connect=con, pen=pen)
    ...
    def _get_range_curve(self):
        ...
        bottom = nanmin(self._dense_y, axis=0)
        top = nanmax(self._dense_y, axis=0)
    ...
    def _get_error_bar(self):
        std = nanstd(self._dense_y, axis=0)
        return pg.ErrorBarItem(x=self.x_data, y=self.__mean,
                               bottom=std, top=std, beam=0.01)

    def _viewport_sample(self, y_min, y_max):
        """Return rows visible in [y_min, y_max], capped at MAX_LINES."""
        y = self._dense_y
        if y_min is not None and y_max is not None:
            # keep rows that have at least one point in the visible y-range
            visible = np.any((y >= y_min) & (y <= y_max), axis=1)
            y = y[visible]
        n = y.shape[0]
        if n > MAX_LINES:
            rng = np.random.default_rng(0)
            idx = np.sort(rng.choice(n, MAX_LINES, replace=False))
            y = y[idx]
        return y

    def update_display_for_range(self, y_min, y_max):
        """Recompute rendered lines for the current viewport; called on zoom/pan."""
        if not self._profiles_added:
            return
        new_display = self._viewport_sample(y_min, y_max)
        x, y, con = self.__get_disconnected_curve_data(new_display)
        self.profiles.setData(x=x, y=y, connect=con)
        x, y, con = self.__get_disconnected_curve_missing_data(new_display)
        self.missing_profiles.setData(x=x, y=y, connect=con)
    ...
    def __get_disconnected_curve_missing_data(y_data):
        m, n = y_data.shape
        x = np.arange(m * n) % n + 1
        arr = y_data.toarray() if sp.issparse(y_data) else np.asarray(y_data)
        y = arr.flatten()
        connect = np.isnan(arr)
        # vectorized: zero out leading-NaN flags (up to first non-NaN per row)
        first_non_nan = np.argmin(connect, axis=1)  # 0 means no leading NaN
...
MAX_FEATURES = 200
MAX_LINES = 10000      # max individual lines rendered (stats still use all data)
...
class OWLinePlot(OWWidget):
    ...
    class Information(OWWidget.Information):
        ...
        too_many_instances = Msg("Too many instances to display. Showing a "
                                 "random sample of {}.".format(MAX_LINES))
    ...
    def _add_graph(self):
        ...
        self._pending_y_range = (None, None)
        self._range_timer = QTimer(self, singleShot=True, interval=100)
        self._range_timer.timeout.connect(self._update_display_range)
        self.graph.view_box.sigRangeChanged.connect(self._on_range_changed)
    ...
    def _on_range_changed(self, _, ranges):
        self._pending_y_range = tuple(ranges[1])
        self._range_timer.start()  # debounce: reset 100 ms countdown

    def _update_display_range(self):
        y_min, y_max = self._pending_y_range
        for group in self.__groups:
            group.update_display_for_range(y_min, y_max)
    ...
    def set_subset_ids(self):
        sub_ids = set(self.subset_data.ids) \
            if self.subset_data is not None else set()
        self.subset_indices = None
        if self.data is not None and sub_ids:
            # set for O(1) membership in _update_sub_profiles
            self.subset_indices = sub_ids & set(self.data.ids)
    ...
    def _update_sel_profiles_and_range(self):
        ...
        if not (self.show_profiles or self.show_range):
            return
        sel_set = set(self.selection) if self.selection else set()
        for group in self.__groups:
            inds = [i for i in group.indices if i in sel_set]
