...
class DistributionBarItem(pg.GraphicsObject):
    ...
    def boundingRect(self):
        if self.expanded:
            ...
            height = sum(self.freqs) if len(self.freqs) > 0 else 0
        else:
            height = max(self.freqs) if len(self.freqs) > 0 else 0
...
class OWDistributions(OWWidget):
    ...
    def set_valid_data(self):
        ...
        # Optimization: for SQL tables, don't download the whole column here.
        # We assume it's valid if metadata exists, pushdown handles NaNs.
    ...
    def _cont_plot(self):
        ...
        if isinstance(self.data, SqlTable):
            dist = distribution.get_distribution(self.data, self.var)
            y, x = np.histogram(dist[0], bins=binning.thresholds, weights=dist[1])
            total = np.sum(dist[1])
        else:
    ...
    def _cont_split_plot(self):
        ...
        if isinstance(self.data, SqlTable):
            ...
            # Use pushdown contingency (row_var=cvar, col_var=var)
            cont = contingency.get_contingency(self.data, self.var, self.cvar)
            # cont is a Contingency object
            values, counts = cont.values, cont.counts
            bins = thresholds
            for val_idx in range(nvalues):
                # counts are row-indexed by class values
                y, _ = np.histogram(values, bins=thresholds, weights=counts[val_idx])
                ys.append(y)
                prior_sizes.append(np.sum(counts[val_idx]))
            total = np.sum(counts)
            fitters = [] # Fitted distributions not supported for SQL pushdown yet
        else:
            data = self.valid_data
            y_bins, bins = np.histogram(data, bins=thresholds)
            fitters = []
            for val_idx in range(nvalues):
    ...
    def recompute_binnings(self):
        ...
        if self.is_valid and self.var.is_continuous:
            ...
            if isinstance(self.data, SqlTable):
                # For SQL, binnings are computed using get_distribution (pushdown)
                # or stats pushdown if we want min/max
                dist = distribution.get_distribution(self.data, self.var)
                column = dist[0]
            else:
                column = self.data.get_column(self.var)
    ...
    def apply(self):
        ...
        if self.is_valid:
            if isinstance(data, SqlTable):
                # For SQL tables, we don't download.
                # We just mark valid_data as the table itself.
                # SqlTable supports len() via pushdown.
                self.valid_data = data
                # But we must avoid calling _get_output_indices which triggers a download.
                pass
            else:
                if self.var.is_discrete:
